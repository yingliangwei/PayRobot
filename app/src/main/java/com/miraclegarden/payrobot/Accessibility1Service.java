...
public class Accessibility1Service extends AccessibilityService {
    ...
    void run(List<AccessibilityNodeInfo> infos) {
        for (AccessibilityNodeInfo node : infos) {
            String bill = StingToMD5(node.getText().toString());
            //判断数据库不存在
            if (!hasData(bill)) {
                Accessibility1Service.this.bill = bill;
                try {
                    long ts = getTime(node.getText().toString());
